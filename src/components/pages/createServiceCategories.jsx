...
import { serviceCategoryCrud } from "../api/ApiRoutesFile";
...
const ServiceCategory = () => {
  ...
  const handleUpdate2 = async (row) => {
    ...
    try {
      const endpoint = `${serviceCategoryCrud}/${row?._id}`;
  ...
  const handleSubmit = async (values) => {
    ...
    try {
      const res = await post(serviceCategoryCrud, data);
      if (res?.success) {
  ...
  const handleSubmit2 = async (values) => {
    ...
    try {
      const endpoint = `${serviceCategoryCrud}/${selectedItem2?._id}`;
          ...
          <Card className="border border-white">
            ...
            <div>
                ...
                <p className="text-gray-500 text-xs md:text-sm mb-0">
                  {"You can add service categories here"}
                </p>
              </div>
              <div className="">
              <button
