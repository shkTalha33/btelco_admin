...
import { faqCat } from "../../api/ApiRoutesFile";
...
const FaqCategory = () => {
  ...
  const handleSubmit = async (values) => {
    ...
    try {
      const res = await post(faqCat, data);
      if (res?.success) {
  ...
  const handleSubmit2 = async (values) => {
    ...
    try {
      const endpoint = `${faqCat}/${selectedItem2?._id}`
    ...
    <>
      <main className="container-fluid p-3 p-lg-5 mx-auto" style={{ minHeight: '90vh' }}>
        <div className="flex justify-between mb-4 gap-2 items-center w-full">
          <h4 className="plusJakara_bold mb-0">Categories List</h4>
          <button onClick={() => setModalOpen(true)} className="bg_primary text_white px-3 py-2 rounded-3 plusJakara_medium">Create Category</button>
        </div>
        <div className="mt-3">
          {loading ? (
      ...
      >
        ...
        <div className="flex justify-center gap-2 w-full my-3">
          ...
          >
            {loadingstatus ? <HashLoader  className="mx-auto"   size={14} color="#fff" /> : "Yes"}
          </button>
        </div>
      </Modal>
    </>
